# Microfluidic time-lapse image processing by Julian Bär

This repository, created by Julian Bär of the [AZinkernagel lab](https://www.usz.ch/en/department/infectiology/research/research-group-annelies-zinkernagel/), is containing code for process time-lapse microscopy images from microfluidic devices. It accompanies the article "Single-cell approach dissecting *agr* quorum sensing dynamics in *Staphylococcus aureus*" by Julian Bär, Mariane Pivard, Samuel G. V. Charlton, Andrew G. Kempchinsky, Alejandro Gómez-Mejia, Giovanni Stefano Ugolini, Srikanth Mairpady Shambat, Eleonora Secchi and Annelies S. Zinkernagel, available currently as a preprint on [bioRxiv](https://www.biorxiv.org/content/10.1101/2024.02.27.582246v1).


## General Information

The repository provides analysis pipelines for both mother-machine (mm) and connected-chamber (cc) experiments. The workflows for these pipelines are largely similar, requiring a metadata CSV file that outlines the images included and monitors pipeline progression. The initial stages, including configuration and preprocessing, are executed in MATLAB, while segmentation and tracking are conducted in Python. Both pipelines commence with the creation of a configuration file, which is essential for storing variables for subsequent scripts. This is followed by preprocessing to identify regions of interest, register the image to counteract stage drift, extract the ROI, and copy these in a new folder, alongside generating an overview movie. This step takes about 5min for one dual-channel 16bit time-lapses with approximately 250 frames on a modern Windows PC and computing time depends on number of filled chambers (for mm), number of image channels and number of frames. Subsequently, StarDist2D segmentation is performed for all images in Python. Segmentation takes about 2min for a single filled chamber of a dual-channel time-lapse with 240 frames for mm and up to 10min for a large connected-chamber time-lapse. For certain mother-machine experiments, cell tracking with DeLTA2.0 is performend which can take up to 15min per filled chamber. The Python jupyter books run in dedicated conda environments. The two pipelines are described below.
...
### Meta data structure:
Columns after `media` will be filled by the script iteratively and should be left empty by the user. The previous ones need/can to be filled by the user. The user can add as many more columns as needed to describe their experimental design.
...
- `channel` : (mandatory) our mother-machine chips had 6 flow channels, this column indicates which flow channel is imaged
...
- `Top1Bot0FRow`: (mandatory) if MiddleRow == 0, this variable indicates if this is a top row (=1) or a bottom row (=0). Filled by script
- `rotation`: (mandatory) automatically detected rotation correction of the image (assumed same rotation for all positions of 1 biological replicate/folder and performs this rotation estimation only once per folder). This is 0 if prerotation was set. Filled by script
...
- `PxinUmY`: (mandatory) spatial calibration factor (should always be the same as the one for X, I guess). Filled by script
- `register`: (mandatory) Turns to "Done" once the s1_preprocessing.m has been run. Filled by script
