...
def getpoles(
    ...
    mypoles = list(props.values())
    mypoles = np.array(mypoles).T
    mypoles = mypoles[:,1:3]
    mypoles = np.round(mypoles)
    clean_poles = []

    for p in range(len(mypoles)):
        clean_poles.append((np.array(mypoles[p], dtype=np.int16), np.array(mypoles[p], dtype=np.int16)))
