...
for maini = 1:length(uniqrep)
    ...
    for checkprog = 1:sum(replind)
        if isempty(char(meta.register(replindx(checkprog)))) && ~strcmp(char(meta.Exclude{checkprog}), 'excl')
            todo = todo + 1;
        end
    end
    if todo == 0
    ...
    dird=fullfile(maindir, alldir(1).name, 'stack1'); %get dir of image
    r = bfGetReader(fullfile(dird, 'frame_t_0.ets')); %able to pull out just one image instead of entire file
    ...
    % create savedirs
    savedir = fullfile(masterdir, savedirname, replicate);
    if ~isfolder(savedir)
        mkdir(savedir)
    end
    vidsavedir = fullfile(savedir, 'movies');
    if ~isfolder(vidsavedir)
        mkdir(vidsavedir)
    end
    reginfofolder = fullfile(savedir, 'reginfofolder');
    ...
    for posi=posL %go over folders
        ...
        % init lists for detection of which type of FOV (middle, top,
        % bottom) and for binarization threshold
        if posi == 1 || ~exist('onerow')
            onerow = zeros(size(posL,2),1); %init 
            onerowtop = zeros(size(posL,2),1); %init
            bthresh = zeros(size(posL,2),1); %init
        end
        ...
        disp(['1st frame processing: Replicate: ', replicate, ' (', num2str(maini, '%02.f'),'/',num2str(length(uniqrep),'%02.f'),...
        ...
        clear r
        try
            r = bfGetReader(fullfile(dird, 'frame_t_0.ets'));
        catch
            if posi == posL(1)
                rotangle = [];
                fix3 = [];
...
%         set prerotation to 0 if it was not defined. Otherwise we'll get
%         errors
        if isempty(meta.prerotate(posind)) || isnan(meta.prerotate(posind))
            meta.prerotate(posind) = 0;
        end
        
        %     read in first image. user can specify if a later image should
        %     be used as first image
        if isempty(meta.firstframe(posind)) || isnan(meta.firstframe(posind))
            ti = 1;
        else
            ti = meta.firstframe(posind);
        end
        ...
        notrotsize = size(bfGetPlane(r, bi));
        bf = imrotate(mat2gray(bfGetPlane(r, bi), thr1), meta.prerotate(posind));
        ...
        if posi == posL(1) || isempty(rotangle) || isnan(rotangle)
            if meta.prerotate(posind)~= 0
                rotangle = 0;
            else
                rotangle = [];
        ...
        [onerow ,reg_box_ref, rotangle, fix3, allchambox, allflipim, posL, onerowtop, skippos, bthresh] =...
            ...
            emptychamuser, shortenchamber, char(meta.Exclude{posind}), rotangleimpose, meta.prerotate(posind), bthresh, notrotsize);
    ...
    for posi=posL %go over folders/position
        ...
        try %init video writer for movie inside savedir
            vidname = fullfile(vidsavedir, ['Pos',num2str(posi,'%02.f'),'.avi']);
        ...
        if ~isnan(meta.MaxFr(posind)) && isnan(meta.Process2(posind))
        ...
        if ~isnan(meta.Process2(posind))
            [parentDir, ~, ~] = fileparts(maindir);
            dird2=fullfile(parentDir, char(meta.replicate2(posind)),  ['_Process_', num2str(meta.Process2(posind)), '_'], 'stack1'); %get dir of image
            clear r2
            ...
            if ~isnan(meta.MaxFr(posind))
                secondprocess = meta.MaxFr(posind);
            else
        ...
        if isempty(meta.firstframe(posind)) || isnan(meta.firstframe(posind))
            ...
            tstart = meta.firstframe(posind);
        end

        updatedreader = false;
        resetcounter = 0;
        for tind=tstart:n_im
            ti=tind;
            if ti==131
                stp=1;
            end
            ...
            if twoprocess && ti>firstprocess && ~updatedreader
                ...
                updatedreader = true;
                resetcounter = ti-1;
                meta.rep2firstframe(posind) = ti;
                meta.rep2delaymin(posind) = meta.rep2startdifferencemin(posind)-((ti-1)*meta.dt(posind));
            end
            ...
            if n_channel == 1
                ...
                bf = imrotate(mat2gray(bfGetPlane(r, bi), thr1), meta.prerotate(posind));
                gfp = imrotate(mat2gray(bfGetPlane(r, gi), thr2), meta.prerotate(posind));
                ...
                if n_channel > 2
                    ...
                    rfp = imrotate(mat2gray(bfGetPlane(r, ri), thr3), meta.prerotate(posind));
            ...
            if ti==tstart
                ...
                [regfail, tform, reg_box_mov, blurval] =...
                    ...
                    rotangle, optimizer, metric, ti, Tregistration, tstart, meta.prerotate(posind), bthresh);
            ...
            if ti == tstart && tstart>1
                ...
                while ti<=tstart
                    for chi=1:size(chambox,1)
                        ...
                        % get image name
                        if savestack
                            genname =[savedir,filesep,'Chambers',filesep,...
            ...
            for chi=1:size(chambox,1)
                ...
                if checkfailedchambers
                    if ti==n_im && skipcham(chi)>maxfailedchamber*n_im
                        rmdir(fullfile(savedir,'Chambers',...
                            ['Pos', num2str(posi,'%02.f')],...
                            ['Chamb',num2str(chi,'%02.f')]), 's');
                    end
                end
            end
            ...
            ov = movieframecreation(bf, gfp, rfp, n_channel,...
                regfail, chambox, col1, col2, col3, burn_chamb,...
                meta, deltaT, posind, ti, twoprocess, updatedreader);
            ...
            if ti==n_im
                ...
                try
                    writetable(meta,...
                        [metaname(1:end-4),'_', char(datetime('today', 'Format','yyyy-MM-dd')), '.csv'],...
                        'Delimiter', ',');
                catch
                end
            end
        ...
        writetable(Tregistration,...
            fullfile(reginfofolder,['Pos', num2str(posi,'%02.f'),'_registration.csv']),...
            'Delimiter', ',')
...
function [onerow ,reg_box_ref, rotangle, fix3, allchambox, allflipim, posL, onerowtop, skippos, bthresh] =...
    frameONEprocessing(bfO, downsampleF, downsampleF_chamberdetect,...
    posi, size_fixed, chamw, posL, onerow, rotangle, fix3, savedir,...
    savestack, maxrotangle, allchambox, allflipim, reg_box_ref, onerowtop,...
    emptychamuser, shortenchamber, excl, rotangleimpose, prerotation, bthresh, notrotsize)
try
    if ~isempty(rotangleimpose)
        rotangle = rotangleimpose;
    end
    ...
    [chambs, chambs1, onerow, rotangle, chambs1bck, onerowtop, bthresh] = ...
        chamberdetectioninitialize(bfO, downsampleF,...
        downsampleF_chamberdetect, posi, onerow, rotangle, maxrotangle, onerowtop,...
        emptychamuser, bthresh, prerotation, notrotsize);
    ...
    chambs1=imdilate(chambs1,strel('line',4,90));
...
%     get distance of prerotationangle to next 90 degrees. if it is
%     anything of it, crop the registration area a bit smaller. The black
%     borders otherwise really mess with accuracy
    distance_to_90 = angleDistance(prerotation);
    if abs(rotangle)>0 || distance_to_90>0 % if there was some rotation applied, crop away some X-border as well to not include black area
        reg_box_ref(posi,1) = round(0.05*reg_box_ref(posi,3));
        reg_box_ref(posi,3) = reg_box_ref(posi,3) - 2*reg_box_ref(posi,1);
    end
    ...
    if onerow(posi) %case of 1 row
        ...
        %get x cordinates
        xbot = sort(props.Centroid(~props.intoprow,1));
        xtop = sort(props.Centroid(props.intoprow,1)); 
        
        %filter the ones that are too close to each other
        ...
        avd = median([dtop;dbot]);
        cutd = quantile([dtop;dbot], 0.9) + 15/range([dtop;dbot]);

        % Given input vectors xbot and xtop, avd, and cutd
        xbot = refine_coordinates(xbot, avd, cutd);
        xtop = refine_coordinates(xtop, avd, cutd);

        %filter the ones that are too close to each other
        sel=logical([diff(xbot)>7;1]);
        xbot=xbot(sel);
        sel=logical([diff(xtop)>7;1]);
        xtop=xtop(sel);
...
%             hold off;
    ...
    fname = [savedir,filesep,'Chambers',filesep,...
        'Pos', num2str(posi,'%02.f')];
    try
        rmdir(fname, 's');
    catch
...
% processing
function [chambs, chambs1, onerow, rotangle, chambs1bck, onerowtop, bthresh] = ...
    chamberdetectioninitialize(bfO, downsampleF,...
    downsampleF_chamberdetect, posi, onerow,...
    rotangle, maxrotangle, onerowtop, emptychamuser, bthresh, prerotation, notrotsize)
...
distance_to_90 = angleDistance(prerotation);
crprot = abs(rotangle)>0;
if isempty(crprot); crprot = 0; end
if crprot || distance_to_90>0 % if there was some rotation applied, crop away some X-border as well to not include black area
    redsize = ceil(downsampleF_chamberdetect*(size(bfO,1) - notrotsize(1)));
%     tmp = imcrop(chambgr, round([0.1*size(chambgr,1), 0.1*size(chambgr,2), 0.8*size(chambgr,1), 0.8*size(chambgr,2)]));
    tmp = imcrop(chambgr, round([redsize, redsize, size(chambgr,1)-2*redsize, size(chambgr,2)-2*redsize]));
    bthresh(posi) = graythresh(tmp)-0.05; %get binarize threshold
else
    redsize = 0;
    bthresh(posi) = graythresh(chambgr)-0.05; %get binarize threshold
end

bfc1=imbinarize(chambgr, bthresh(posi));  %binarize imshow(bfc1)
if crprot || distance_to_90>0 % if there was some rotation applied, crop away some X-border as well to not include black area
    bfc1 = imcrop(bfc1, round([redsize, redsize, size(chambgr,1)-2*redsize, size(chambgr,2)-2*redsize]));
end

percb = sum(bfc1(:))/(size(bfc1,1)*size(bfc1,2)); %check if binarization was okish

if percb < 0.11
    bthresh(posi) = bthresh(posi)-0.1;
    bfc1 = imbinarize(chambgr, bthresh(posi));  %binarize
    percb = sum(bfc1(:))/(size(bfc1,1)*size(bfc1,2)); %check if binarization was okish
    if crprot || distance_to_90>0 % if there was some rotation applied, crop away some X-border as well to not include black area
        bfc1 = imcrop(bfc1, round([redsize, redsize, size(chambgr,1)-2*redsize, size(chambgr,2)-2*redsize]));
    end
...
objposbased = 1;
if objposbased
    topdown = imdilate(bfc1, strel('rect', [1, 8]));
    topdown = imfill(topdown,"holes"); % fill holes
    props = regionprops(topdown, 'PixelIdxList', 'Area');
    
    for ind=1:size(props,1)
        if props(ind).Area<200 %keep only 5+px long skeleton objects
            topdown(props(ind).PixelIdxList)=0;
        end
    end
    
    props = regionprops('table', topdown, 'Area', 'Centroid');
    [val, ~] = maxk(vertcat(props.Area), 2);
    if length(val)==1
        potonerow = 1;
    else
        if abs(diff(val)) > 5000
            potonerow = 1;
        else
            potonerow = 0;
        end
    ...
    if potonerow
        [~, bigi] = max(props.Area);
        ypos = props.Centroid(:,2);
        yposBig = props.Centroid(bigi,2);
        ystart = round(max(yposBig-round(size(bfc1,1)/4), 1));
        yend = round(min(yposBig+round(size(bfc1,1)/4), size(bfc1,1)));
        halfb = bfc1(ystart:yend,:); % get percentage foreground in upper half of image
        percb = sum(halfb(:))/(size(halfb,1)*size(halfb,2)); %check if binarization was okish
        ypos(bigi) = [];
        if mean(ypos) < yposBig
            percbup = 1; percbbot = 0.01;
        else
            percbup = 0.01; percbbot = 1;
        end
    end
...
hithrbin = 0.3;
if crprot || distance_to_90>0 % if there was some rotation applied, crop away some X-border as well to not include black area
    lothrbin = lothrbin*(100-distance_to_90-7)/100;
    hithrbin = hithrbin*(100+distance_to_90+7)/100;
end
% adjust threshold for binarization if too small or big
if percb<lothrbin
    while percb<=lothrbin
        bthresh(posi) = bthresh(posi)-0.1;
        bfc1 = imbinarize(chambgr, bthresh(posi));  %binarize
        if crprot || distance_to_90>0 % if there was some rotation applied, crop away some X-border as well to not include black area
            bfc1 = imcrop(bfc1, round([redsize, redsize, size(chambgr,1)-2*redsize, size(chambgr,2)-2*redsize]));
...
elseif percb>hithrbin
    while percb>=hithrbin
        bthresh(posi) = bthresh(posi)+0.05;
        bfc1 = imbinarize(chambgr, bthresh(posi));  %binarize
        if crprot || distance_to_90>0 % if there was some rotation applied, crop away some X-border as well to not include black area
            bfc1 = imcrop(bfc1, round([redsize, redsize, size(chambgr,1)-2*redsize, size(chambgr,2)-2*redsize]));
        end
...
if posi==1 || isempty(rotangle)
    bfc1 = imdilate(bfc1, true(4,9)); % dilate
...
bfc2 = bwmorph(imdilate(bfc2,strel('line',6,90)),'bridge');%close gaps
...
chambs1 = imerode(chambs, strel('line', 3, 0)); %remove potential connections of chambers and number label
chambs1=imdilate(chambs1,strel('line',12,00)); %dilate with a straight horizontal line
chambs1=imfill(chambs1,'holes'); %fill holes
chambs1 = bwareaopen(chambs1, 1200); %remove "small" objects
...
[~, indx] = maxk(vertcat(props.Area), 2);
if props(indx(2)).MinorAxisLength / props(indx(2)).MajorAxisLength < 0.095 && props(indx(2)).Extent < 0.9
    chambs1(props(indx(2)).PixelIdxList)=0;
end
for ind=1:size(props,1)
    ext=abs(props(ind).Extent);
    if ext<0.6 %keep only 5+px long skeleton objects
...
% resize output images if necesarry
if crprot || distance_to_90>0 % if there was some rotation applied, crop away some X-border as well to not include black area
    temp = false(size(imresize(bfO,downsampleF_chamberdetect)));
    startx = ceil((size(temp,1)-size(chambs,1))/2);
    if startx==0; startx=1;end
    endx = ceil((size(temp,1)-size(chambs,1))/2);
    starty = ceil((size(temp,2)-size(chambs,2))/2);
    if starty==0; starty=1; end
    endy = ceil((size(temp,2)-size(chambs,2))/2);
    temp(startx:end-endx, ...
        starty:end-endy) = chambs;
%     temp(redsize:end-redsize, redsize:end-redsize) = chambs;
    chambs = temp;

    temp = false(size(chambs));
    temp(startx:end-endx, ...
        starty:end-endy) = chambs1;
    chambs1 = temp;

    temp = false(size(chambs));
    temp(startx:end-endx, ...
        starty:end-endy) = chambs1bck;
    chambs1bck = temp;
end
...
function [regfail, tform, reg_box_mov, blurval] =...
    ...
    optimizer, metric, ti, Tregistration, tstart, prerotation, bthresh)
...
if blurval>OOFcutoff % if OOF, skip basically everything else
    ...
    bfsmall = imresize(bf,downsampleF_chamberdetect); %downsample
    bfsmall = smallimgprocessing(bfsmall); %get filtered grayscale image
    
    bfc=imbinarize(bfsmall, bthresh(posi));  %binarize
    percb = sum(bfc(:))/(size(bfc,1)*size(bfc,2)); %check binarization
...
%         end
    if percb>hithrbin
        bthreshtemp = graythresh(bfsmall)+0.05; %get binarization threshold
        if bthreshtemp>bthresh(posi)
            bfc = imbinarize(bfsmall, bthreshtemp);  %binarize
        else
            bfc = imbinarize(bfsmall, bthresh(posi)+0.1);  %binarize
        end
    end
%     bfc = bfc1; %reassign
    ...
    sft=ceil(size(bfc1,1)*0.025); %padding by this many pixels
    if onerow(posi) %onerow case
        try
            ...
            if size(props,1) > 1
                ...
                if onerowtop(posi)
                    ...
                    reg_box_mov(2) = ceil(props.BoundingBox(1,2) + props.BoundingBox(4)); %end of chamber area
    ...
    distance_to_90 = angleDistance(prerotation);
    if abs(rotangle)>0.01 || distance_to_90>0.01 % if there was some rotation applied, crop away some X-border as well to not include black area
        reg_box_mov(1) = round(0.05*reg_box_mov(3));
        reg_box_mov(3) = reg_box_mov(3) - 2*reg_box_mov(1);
    end
    ...
    if size(mov3,1) < 16 || size(mov3,2) < 16
        reg_box_mov = reg_box_ref(posi,:);
        reg_box_mov(2) = reg_box_mov(2)-20;
        reg_box_mov(4) = reg_box_mov(4)+40;
        if reg_box_mov(1)<1; reg_box_mov(1) = 1; end
        if reg_box_mov(2)<1; reg_box_mov(2) = 1; end
        mxy=size(imresize(bf,downsampleF),2);
        if reg_box_mov(2)+reg_box_mov(4)>mxy
            reg_box_mov(4) = mxy-reg_box_mov(2);
        end
        mov3=imcrop(imresize(bf,downsampleF),reg_box_mov);
    end
...
%             abs(reg_box_ref(posi,2)-reg_box_mov(2)) > 60
%         reg_box_mov = reg_box_ref(posi,:);
...
%     end
    ...
    if ti > tstart+1 && (~isnan(Tregistration.x_reg(end)) || ~isnan(Tregistration.x_reg(end-1)))
        tform = transltform2d;
        if ~isnan(Tregistration.x_reg(end))
            tform.T(3,1) = Tregistration.x_reg(end);
            tform.T(3,2) = Tregistration.y_reg(end);
        else
            tform.T(3,1) = Tregistration.x_reg(end-1);
            tform.T(3,2) = Tregistration.y_reg(end-1);
        end
...
if blurval>OOFcutoff
    ...
    if correg < 0.5 || isnan(correg) %cross-correlation
        reg_box_movalt = reg_box_ref(posi,:);
        reg_box_movalt(2) = reg_box_movalt(2)-30;
        reg_box_movalt(4) = reg_box_movalt(4)+60;
        if reg_box_movalt(2)+reg_box_movalt(4)>size(imresize(bf,downsampleF),2)
            reg_box_movalt(4) = ceil(size(imresize(bf,downsampleF),2)-reg_box_movalt(2));
        end
...
function ov = movieframecreation(bf, gfp, rfp, n_channel,...
    regfail, chambox, col1, col2, col3, burn_chamb, meta, deltaT,...
    posind, ti, twoprocess, updatedreader)
...
if n_channel>1
    ...
    bchan = bf;
    bchan = bchan + gfp*col1(3);
    bchan = bchan + rfp*col2(3);
...
if twoprocess && updatedreader
    time1 = time1+meta.rep2delaymin(posind);
end

% Step 1: Calculate hours, minutes, and seconds
hours = floor(time1 / 60);
minutes = floor(mod(time1, 60));
seconds = round((time1 - floor(time1)) * 60);

% Step 2: Format the output based on the presence of seconds
if seconds > 0
    % Format as HH:MM:SS
...
ov = insertText(ov, [30, 50], timeString, 'FontSize', 80, 'BoxColor', tcol);
end
...
function coordinates = refine_coordinates(coordinates, avd, cutd)
    % Calculate initial differences between consecutive elements
    dcoords = diff(coordinates);
    
    % Loop until all consecutive differences are within the allowed distance
    attempts = 0;
    while any(dcoords(1:end-1) > cutd) && attempts < 8
        attempts = attempts +1;
        % Find indices where the difference exceeds the allowed distance
        chamcorr = find(dcoords > cutd);
        
        % Merge consecutive indices in chamcorr to process as single blocks
        gaps = [];
        start_idx = chamcorr(1);
        for i = 2:length(chamcorr)
            if chamcorr(i) > chamcorr(i-1) + 1
                % If not consecutive, finalize the current block
                gaps = [gaps; start_idx, chamcorr(i-1)];
                start_idx = chamcorr(i);
            end
        end
        % Add the last block
        gaps = [gaps; start_idx, chamcorr(end)];
        
        % Initialize a list to collect new points
        new_points = [];
        torem = [];
        
        % Iterate over each gap block
        for ni = 1:size(gaps, 1)
            % Determine the start and end points of the block
            start_point = coordinates(gaps(ni, 1));
            end_point = coordinates(gaps(ni, 2) + 1);
            
            % Calculate the number of new points needed
            total_gap = end_point - start_point;
            num_new_points = max(0, round(total_gap / avd) - 1); % Avoid infinite loop
            
            % Generate new points linearly spaced between start and end points
            if num_new_points > 0
                pointstoadd = linspace(start_point + avd, end_point - avd, num_new_points);
                new_points = [new_points, pointstoadd];
                torem = unique([torem; find(coordinates >= min(pointstoadd) & coordinates <= max(pointstoadd))]);
            end
        end
        
        % Remove overlapping points and add new points to the vector
        coordinates(torem) = [];
        coordinates = sort([coordinates; new_points']);
        
        % Recalculate the differences between consecutive elements
        dcoords = diff(coordinates);
    end
end
